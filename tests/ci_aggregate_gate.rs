//! One check gates a pull request, and it stands for every job (#105).
//!
//! Branch protection names checks, and a check is a job name. This
//! repository named six of them by hand — `fmt`, `qemu-validation`, the
//! three `test / <os>` matrix legs and `test (release)` — which means the
//! list has to be edited whenever a job is renamed, split into a matrix leg
//! or added, and until someone does, the new work is required by nobody. A
//! fourth entry in ci.yml's `os:` matrix would report on every pull request
//! and gate none of them, which is the state the sibling rust-img-qcow2 was
//! found in. The opposite spelling is worse: a required check no job
//! produces reads as permanently pending rather than failed, and with
