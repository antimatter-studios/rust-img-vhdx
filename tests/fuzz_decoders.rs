...
//! here from then on.
//!
//! # Why the corpus is mostly sections, and only one whole image
//!
...
//! image.
//!
//! A VHDX is also 8 MB before it holds anything -- a 1 MB log region,
//! the metadata and BAT regions, and alignment between them -- and that
//! is the format's floor rather than a choice. So the header, region
//! table, metadata region and log region are cut out of three images
//! and seeded on their own at 64 KiB each, and exactly ONE whole image
//! is committed for the target that has to open one. Git stores it in
//! about 17 KB because it is nearly all zeros.
//!
//! That image gets a much smaller case budget than the section targets,
//! which is what `cases` on each target is for: every case copies the
//! whole buffer, and 16 MB per case at the section targets' rate would
//! be minutes rather than seconds.
//!
//! # The corpus is an oracle
//!
//! `qemu-io` wrote `A` at offset 0 and `B` at 512 KiB, both inside
//! block 0, and left block 1 unallocated. `the_corpus_reads_back_what_qemu_img_wrote` checks
    ...
    vec![
        ...
        Target {
            ...
            run: |b| {
                ...
                guid[..take].copy_from_slice(&b[..take]);
...
///
/// `qemu-io` wrote `A` at offset 0 and `B` at 512 KiB and left block 1
/// unallocated, so this crate must return exactly that.
///
/// It also keeps the seed from quietly becoming unopenable: a mutation
/// of an image that no longer opens is also unopenable, so the corpus
...
fn the_corpus_reads_back_what_qemu_img_wrote() {
    ...
    const SECOND_AT: u64 = 512 << 10;
    const RUN: usize = 8_000;
    const HOLE_AT: u64 = 1 << 20;

    let images = seeds("image");
    assert!(
        !images.is_empty(),
        "the image corpus is empty; there is nothing to open"
    );
    ...
    for (name, bytes) in images {
        ...
        assert!(
            ...
            "{name}: the run at {FIRST_AT} is not the one qemu-io wrote"
        );
        ...
        assert!(
            ...
            "{name}: the run at {SECOND_AT} is not the one qemu-io wrote"
        );

        // The hole reads as zeros rather than as whatever was last in
        // that block, which is the property an unallocated BAT entry has
        // and the one a BAT indexing error breaks.
        ...
        Err(mpsc::RecvTimeoutError::Timeout) => {
            ...
            let _ = writeln!(
                ...
                "\nhung: no progress for {:?} at {}\n\
                 A decoder did not return. A log chain whose entries point back at one \
                 another looks exactly like this.",
                DEADLINE,
                describe(&current),
            );
