...
//!
//! `include/vhdx.h` said "Link with libam_img_vhdx.a" for as long as
//! the file existed, and cargo has never produced that name: `[lib]
//! name` is `vhdx`, so the artefact is `libvhdx.a` -- which is exactly
...
fn the_header_scan_finds_library_names_in_prose() {
    ...
    assert_eq!(
        libraries_named(" * Link with libam_img_vhdx.a alongside\n"),
